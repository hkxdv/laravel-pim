...
final class ProductController extends Controller
{
    ...
    public function search(Request $request): JsonResponse
    {
        ...
        $isActive = $request->has('is_active')
            ? (bool) $request->boolean('is_active') : null;
        $sortField = (string) $request->query(
            'sort_field',
        ...
        $sortDir = mb_strtolower(
            (string) $request->query(
                'sort_dir',
                (string) $request->query('sort_direction', 'desc')
            )
        ) === 'asc' ? 'asc' : 'desc';
        $perPage = (int) $request->query('per_page', 15);
        ...
        Log::info('Inventory products.search', [
            'mode' => $mode,
            'engine' => $engine::class,
            'params' => $params,
            'ip' => $request->ip(),
            'user_id' => $request->user()?->id,
            'req_header_x_search_mode' => $request->headers
                ->get('X-Search-Mode'),
        ]);
    ...
    public function store(StoreInventoryProductRequest $request): JsonResponse
    {
        $this->requireStaffUser($request);
