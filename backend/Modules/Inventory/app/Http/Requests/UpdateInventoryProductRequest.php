...
final class UpdateInventoryProductRequest extends FormRequest
{
    ...
    /**
     * Nombres amigables para atributos.
     *
    ...
    public function attributes(): array
    {
        ...
        if (is_string($slug) && $slug !== '') {
            $path = resource_path(
                sprintf('product-specs/%s.json', $slug)
            );
            ...
            if (is_file($path) && is_readable($path)) {
                ...
                $definition = is_array($dataArr['definition'] ?? null)
                    ? $dataArr['definition'] : [];
                $groups = is_array($definition['spec_groups'] ?? null)
                    ? $definition['spec_groups'] : [];
                ...
                foreach ($groups as $group) {
                    ...
                    $fields = is_array($group['fields'] ?? null)
                        ? $group['fields'] : [];
                    if ($fields === []) {
                        continue;
                    }
    ...
    /**
     * Genera reglas dinámicas para `metadata` según el `_spec_slug`.
     *
     * @return array<string, array<int, string>>
     */
    private function buildSpecRules(): array
    {
        ...
        foreach ($groups as $group) {
            ...
            foreach ($fields as $field) {
                ...
                $constraints = is_array($field['constraints'] ?? null)
                    ? $field['constraints'] : [];
                $rule = [];
                ...
                if ($constraints !== []) {
                    ...
                    if (
                        isset($constraints['min'])
                        && is_numeric($constraints['min'])
                    ) {
                        $rule[] = 'min:'.(int) $constraints['min'];
                    }

                    if (
                        isset($constraints['max'])
                        && is_numeric($constraints['max'])
                    ) {
                    ...
                    if (
                        ...
                        && is_string($constraints['pattern']) && $constraints['pattern'] !== ''
                    ) {
                    ...
                    if (
                        $type === 'file'
                        && isset($constraints['max_size_kb'])
                        && is_numeric($constraints['max_size_kb'])
                    ) {
                        $rules[sprintf(
                            'metadata.%s.size_kb',
                            $fid
                            ...
                            'sometimes',
                            'integer',
                            'max:'.(int) $constraints['max_size_kb'],
                        ];
                ...
                if ($type === 'select') {
                    $opts = is_array($field['options'] ?? null)
                        ? $field['options'] : [];
                    if ($opts !== []) {
