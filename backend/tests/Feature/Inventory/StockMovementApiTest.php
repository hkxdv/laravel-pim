...
it(
    'creates an IN stock movement and increments stock',
    function () {
...
it(
    'prevents OUT movement when stock is insufficient',
    function () {
        $user = authStaffForMovements();
        $this->actingAs($user, 'staff');
        $headers = apiAuthHeadersFor($user);
        ...
        try {
            $res = $this->postJson('/api/v1/inventory/stock-movements', $payload, $headers);
...
it(
    'creates an ADJUST movement that sets exact stock level',
    function () {
        $user = authStaffForMovements();
        $this->actingAs($user, 'staff');
        $headers = apiAuthHeadersFor($user);
